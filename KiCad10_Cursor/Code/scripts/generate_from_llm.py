...
def _run_kicad_generation(data: dict, json_path: str, output_dir: str) -> int:
    ...
    # Optional: LLM placement pass (positions only; wiring remains deterministic).
    placement_map: dict | None = None
    _llm_place = os.environ.get("SCHEMATIQ_LLM_PLACE") or os.environ.get("CHIPCHAT_LLM_PLACE", "")
    if _llm_place.lower() in ("1", "true", "yes"):
        try:
            from src.lib.schematic_placement_llm import propose_placements

            print("\n--- (LLM) Placement pass ---")
            placement_report = propose_placements(data)
            placement_map = placement_report.get("sheets") or {}
            os.makedirs(REPORT_DIR, exist_ok=True)
            base = os.path.splitext(os.path.basename(json_path))[0]
            outp = os.path.join(REPORT_DIR, f"{base}_placement.json")
            with open(outp, "w", encoding="utf-8") as pf:
                json.dump(placement_report, pf, indent=2)
                pf.write("\n")
            print(f"(placement) report: {outp}")
        except Exception as e:
            print(f"(placement) skipped due to error: {e}")
    ...
    for sheet_def in sheets:
        ...
        try:
            schematic_generator.generate_from_json(
                output_path, json_path, sheet_name=sheet_name, placements=placement_map
            )
        except Exception as e:
    ...
    from src.lib.schematic_verifier import verify_schematic, parse_kicad_sch, format_report
