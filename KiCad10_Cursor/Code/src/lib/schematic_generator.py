...
def _get_official_kicad_symbols_dir():
    """Official KiCad symbols: ``component_database/kicad-symbols`` or ``KICAD_Library/kicad-symbols``."""
    r = official_kicad_symbols_root()
    if r:
        return r
    base = os.path.dirname(os.path.dirname(os.path.dirname(
        os.path.dirname(os.path.abspath(__file__)))))
...
def _embed_passive_symbol(schematic_data, symbol_type):
    ...
    if packed_lib_name and packed_sym:
        packed_dir = _get_official_kicad_symbols_dir()
        ...
        if os.path.exists(packed_lib_file):
            ...
            if lib_id:
                return lib_id
        symdir = official_library_symdir_path(packed_lib_name)
        if symdir and find_unpacked_symbol_file(symdir, packed_sym):
            lib_id = kicad_api.embed_symbol_from_file(
                schematic_data,
                packed_sym,
                library_path=symdir,
                lib_prefix=packed_lib_name,
            )
            if lib_id:
                return lib_id
    ...
    for lib_nick, sym_names in needed.items():
        ...
        for sym_name in sym_names:
            ...
            if os.path.exists(lib_file):
                ...
                if sf:
                    result = kicad_api.embed_symbol_from_file(
                        schematic_data, sym_name, library_path=symdir, lib_prefix=lib_nick
                    )
            if result:
...
def generate_from_json(output_path, json_path, sheet_name="BME280_Sensor", *, placements: dict | None = None):
    ...
    for idx, comp in enumerate(main_comps):
        ...
        if not lib_id and ":" in symbol_lookup:
            ...
            packed_lib = official_library_packed_path(lib_name)
            if packed_lib and os.path.exists(packed_lib):
                alt = sym_name
                ...
                udir = official_library_symdir_path(lib_name)
                if udir:
                    sfile = find_unpacked_symbol_file(udir, sym_name)
                    if sfile:
                        alt = sym_name
                        maybe = symbol_resolver.resolve_in_packed_library(sfile, sym_name)
                        if maybe:
                            alt = maybe
                        resolved_name = alt
                        lib_id = kicad_api.embed_symbol_from_file(
        ...
        if not lib_id:
            ...
            if resolved:
                ...
                if kind == "packed":
                    ...
                    prefix = symbol_lookup.split(":", 1)[0].strip() if ":" in symbol_lookup else "SchematIQ"
                    lib_id = kicad_api.embed_symbol_from_file(
                        schematic_data,
                        resolved_name,
                        library_path=udir,
    ...
    if power_nets:
        ...
        if power_lib and os.path.exists(power_lib):
            ...
            if pdir and find_unpacked_symbol_file(pdir, "PWR_FLAG"):
                pwr_flag_id = kicad_api.embed_symbol_from_file(
                    schematic_data, "PWR_FLAG", library_path=pdir, lib_prefix="power"
                )
        if pwr_flag_id:
