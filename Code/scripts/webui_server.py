...
def _load_components_db() -> dict[str, Any]:
    path = _ROOT.parent / "component_database" / "components.json"
    with open(path, encoding="utf-8") as f:
        return json.load(f)


def _system_prompt() -> str:
    # Reuse the same prompt content as the CLI playground for consistency.
    from scripts.prompt_playground import SYSTEM_PROMPT

    components_db = _load_components_db()
    return SYSTEM_PROMPT.format(components_db=json.dumps(components_db, indent=2))
...
def _resolve_json_path(raw: str) -> Path:
    ...
    if not p.is_absolute():
        p = (_ROOT / p).resolve()
    if not p.exists():
        raise HTTPException(status_code=404, detail=f"json_path not found: {p}")
    return p
