...
If you need a part NOT in this database, you MUST use a real KiCad symbol that \
exists in the official KiCad libraries. Use the **"Library:SymbolName"** format.

**CRITICAL: Only use parts you are CERTAIN exist in KiCad's default libraries.** \
When in doubt, prefer well-known parts from these common families:
- **Regulators**: `Regulator_Linear:LM1117DT-3.3`, `Regulator_Linear:AP1117-15`, \
`Regulator_Linear:LM7805_TO220` (NOT custom part numbers like ADP124 or TPS628438)
- **Op-amps**: `Amplifier_Operational:LM741`, `Amplifier_Operational:TL071`, \
`Amplifier_Operational:MCP6001R`
- **Connectors**: `Connector_Generic:Conn_01x02`, `Connector_Generic:Conn_01x04`, \
`Connector:USB_B_Micro`, `Connector:Conn_ARM_JTAG_SWD_10`
- **Passives**: `Device:R`, `Device:C`, `Device:LED`, `Device:Crystal`
- **MCUs**: `MCU_Nordic:nRF52840`, `MCU_ST:STM32F103C8Tx`
- **Sensors**: `Sensor:BME280`, `Sensor:DHT11`
- **Timers**: `Timer:NE555P`

Do **NOT** invent part names. If a part family you want (like ADP124) isn't in these \
common KiCad libraries, pick the CLOSEST well-known alternative that serves the same \
function (e.g. use `Regulator_Linear:LM1117DT-3.3` instead of a custom ADP124).

**Do NOT** invent placeholder part names like `OpAmp_Single` or `GENERIC_OPAMP`. \
**Do NOT** use ```new_component``` for generic placeholders.
